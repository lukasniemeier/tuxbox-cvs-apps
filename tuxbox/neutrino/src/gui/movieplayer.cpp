/*
  ...
  $Id: movieplayer.cpp,v 1.205 2012/04/13 12:15:21 rhabarber1848 Exp $
...
// default offset for jumping 1 second forward/backwards in file
...
void *mp_playFileMain(void *filename)
{
	...
	do
	{
		...
		//-- calculate offset for jumping 1 second forward/backwards in file --
		...
		if ( (q = new TPtrQueue(ctx)) != NULL ) 
		{
			...
			if (q->startReader()) 
			{
				     ...
				     (g_playstate >= CMoviePlayerGui::PLAY) )
				{
					...
					if (ctx->fileSize > 0)
					{
						g_percent = (ctx->pos * 100) / ctx->fileSize;
						if (last_percent != g_percent)
						{
							last_percent = g_percent;
							CLCD::getInstance()->showPercentOver(last_percent, true, CLCD::MODE_MOVIE);
...
void CMoviePlayerGui::PlayFile (int parental)
{
	...
	do
	{
		...
		if(isBookmark)
		{
			...
			sel_filename     = startfilename.substr(startfilename.rfind("/") + 1);
			update_info      = true;
			start_play       = true;

			// get the movie info handle
			cMovieInfo.clearMovieInfo(&movie_info);
			movie_info.file.Name = filename;
			p_movie_info = (cMovieInfo.loadMovieInfo(&movie_info)) ? &movie_info : NULL;
		}
		...
		//-- (new) item in filelist selected --
		//-------------------------------------
		if(filelistIndex > -1)
		{
			filename      = filelist[filelistIndex].Name.c_str();
			sel_filename  = filelist[filelistIndex].getFileName();
			filelistIndex = -1;
			update_info   = true;
		...
		switch(msg)
		{
			...
			case NeutrinoMessages::SHOW_INFOBAR:
				{
			...
			case CRCInput::RC_ok:
				...
				//-- show movie info --
			case NeutrinoMessages::SHOW_EPG:
				if(p_movie_info != NULL)
...
CMoviePlayerGui::PlayStream (int streamtype)
{
	...
	do
	{
		...
		{
			std::string infobar_title = (title.empty()) ? sel_filename : title;

			int stream_length = VlcGetStreamLength();
			int stream_time = VlcGetStreamTime();

			int ac3state = (ac3) ? CInfoViewer::AC3_ACTIVE : CInfoViewer::NO_AC3;
...
{	
	...
	return imageinfo.getModulVersion("1.","$Revision: 1.205 $");
}
...
void CMoviePlayerGui::showHelpTS()
{
	...
	helpbox.addLine(NEUTRINO_ICON_BUTTON_HOME, g_Locale->getText(LOCALE_MOVIEPLAYER_TSHELP1));
	...
	helpbox.addLine(NEUTRINO_ICON_BUTTON_2, g_Locale->getText(LOCALE_MOVIEPLAYER_TSHELP14));
	...
	helpbox.addLine(NEUTRINO_ICON_BUTTON_8, g_Locale->getText(LOCALE_MOVIEPLAYER_TSHELP15));
	helpbox.addLine(NEUTRINO_ICON_BUTTON_9, g_Locale->getText(LOCALE_MOVIEPLAYER_TSHELP11));
	helpbox.addLine(NEUTRINO_ICON_BUTTON_0, g_Locale->getText(LOCALE_MOVIEPLAYER_TSHELP16));
	...
	helpbox.addLine(NEUTRINO_ICON_BUTTON_TOP, g_Locale->getText(LOCALE_MOVIEPLAYER_TSHELP22));
